...
char file_dir[] = "GFILES";
char file_list[] = "file.lst";
char help_dir[] = "syshelp";
char help_list[] = "file.lst";
...
void gfiles_section(char *str,char *name,int portnum)
 {
   file_list_section(file_dir,file_list,"GTalk Online Files Library");
 };

void help_section(char *str,char *name,int portnum)
 {
   file_list_section(help_dir,help_list,"GTalk System Help Library");
 };
...
void log_user_is_leaving(int portnum,const char *log_file)
 {
   ...
   num = user_lines[portnum].user_info.number;
   if (num<0)
