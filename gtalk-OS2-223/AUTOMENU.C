...
{
   int portnum = tswitch;
   int old_code = special_code(1,portnum);
              ...
              struct return_info_struct *rinfo,struct dirty_rect_struct *dirty)
{
    ...
    switch(command)
    {
        ...
        case FIELD_EDIT:
                ...
                get_string_cntrl_pos(data,info.screen_len,0,0,1,0,0,0,0,0);

                /*
                print_masked_string(data,info.mask_char);
                get_string_cntrl_pos(data,info.screen_len,info.mask_char,0,1,0,0,0,0,strlen(data));
