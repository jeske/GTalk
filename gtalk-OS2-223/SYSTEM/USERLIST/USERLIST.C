    ...
    for (loop2=0;loop2<numusers;loop2++)
     {
      ...
      if (fread(&user_ptr,sizeof(struct user_data_record),1,fileptr))
        {
           if ((user_ptr.user.info.number==(loop2)))
           {
             int loop;

             printf("%03d  %c%s%c    %s \n",user_ptr.user.info.number,user_ptr.class_mod.info.staple[0],
                  user_ptr.user.info.handle,user_ptr.class_mod.info.staple[1],user_ptr.user.info.password);
