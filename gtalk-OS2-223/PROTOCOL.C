...
void add_pathnames(char *fullname, char *directory, char *filename, int maxlen)
{
  ...
  if (directory)
  {
    if (*directory)
    {
      strncpy(fullname,directory,maxlen-1);
      fullname[maxlen] = 0;
      len = strlen(directory);
      if (directory[len-1] != '\\')
        strcat(fullname,"\\");
      strncat(fullname,filename,maxlen-strlen(fullname));
#ifdef DEBUG
      sprintf(s,"Full name: %s",fullname);
      print_str_cr_to(s,0);
#endif
      return;
    }
  }
  strncpy(fullname,filename,maxlen);
  fullname[maxlen+1] = 0;
}
...
{
	send_chars(tswitch,buffer,length);
}
...
int get_timeout(unsigned int wait_time)
{
	time_t cur_time;
	int ch;
	...
	while((time(NULL)-cur_time)<wait_time);


	return (-1);
};

int wait_for_sending_state(unsigned int wait_time, int state)
{
	unsigned int cur_time;
	int ch;

	if (char_in_buf(tswitch)==state)
	  return (state);
...
int receiver_driven_start(int mode)
{
    ...
    while (attempts++ < 20)
    {
      ...
      empty_inbuffer(tswitch);
#ifdef DEBUG
      sprintf(s,"Sending start character %03d mode %03d",mode_letter,mode);
      print_str_cr_to(s,0);
#endif
      send_short(mode_letter);
	  wait_for_xmit(tswitch,30);
	  cur_timer = time(NULL);
	  while (((time(NULL) - cur_timer) < SMALL_TIMEOUT_TICKS) &&
	   (!(charbuf=char_in_buf(tswitch))))
		  task_sleep_timeout(250l);
      if (charbuf) return (0);
	}    return (-1);
}
...
int check_cancel(void)
{
  ...
  {
	cur_time = time(NULL);
	while ( ((time(NULL)-cur_time)<SMALL_TIMEOUT_TICKS)
			&& ((ch=time_char()) == -1) )
			  task_sleep_timeout(1000l);
	if (ch == -1) return (0);
	if (ch == CAN_CHR) count++;
...
int proto_open_file(char *filename, FILE **fileptr, char *attributes)
{
  if (*attributes == 'w')
  {
    if (*fileptr=g_fopen(filename,"rb","PROTO#0"))
    {
      g_fclose(*fileptr);
      return (-1);
    }
...
int send_xmodem_protocol(char *filename, int length,struct transfer_struct *this_xfer)
{
  int result;
  int packetno = 1;
  int attempts;
  ...
  this_xfer->filename = filename; /* report xfer filename */
  this_xfer->file_number++;
  this_xfer->total_bytes = maxlength;

  while (curlength < maxlength)
  {
    this_xfer->current_packet_no = packetno;
    this_xfer->current_bytes = curlength;
    ...
    if (not_sent)
    {
        ...
        g_free(fileptr);
        g_fclose(fileptr);
        this_xfer->event=PROTO_ERROR_END;
        return (1);
    }
    curlength += length;
    packetno++;
...
void proto_remove_file(char *filename, FILE *fp)
{
  lock_dos(3371);
  remove(filename);
  g_fclose(fp);
...
int receive_xmodem_protocol(char *directory, char *filename, int crc_type, int mode,struct transfer_struct *this_xfer)
{
  int result;
  int packetno = 1;
  int gotpacketno;
  ...
  print_str_cr_to("Started protocol",0);
#endif
  add_pathnames(fullname,directory,filename,MAX_FILENAME_LENGTH);
  if (proto_open_file(fullname,&fileptr,"wb"))
  {
    massive_cancel();
    return (1);
  }
  ...
  {
    massive_cancel();
    proto_remove_file(fullname,fileptr);
    return (1);
  }
#ifdef DEBUG
  print_str_cr_to("Opened file",0);
...
int send_ymodem_protocol(char *filename, int mode,struct transfer_struct *this_xfer)
{
  int result;
  int packetno = 0;
  int attempts;
...
int receive_ymodem_protocol(char *directory, char *filename, int mode, int *last_file,struct transfer_struct *this_xfer)
{
  int result;
  int packetno = 0;
  int gotpacketno;
  ...
  this_xfer->event=PROTO_FILE_START;
  this_xfer->file_number++;
  while (flag)
  {
    this_xfer->current_packet_no = packetno;
    ...
    while (not_received && (attempts++ < 20))
    {
      if (not_got_mode_type)
      {
        ...
        {
          massive_cancel();
          if (fileptr) proto_remove_file(fullname,fileptr);
          g_free(buffer);
          return (-1);
        }
      }
...
int send_ymodem_batch(char **filenames, int count, int mode,struct transfer_struct *this_xfer)
{
  int i;
  int abort = 0;
  char s[80];
  ...
  for (i=0;i<count;i++)
  {
    if (send_ymodem_protocol(filenames[i],mode,this_xfer))
    {
      abort = 1;
      break;
    }
  ...
  if (!abort)
  {
    ...
    abort=send_ymodem_protocol("",mode,this_xfer);
  }
  return (abort);
}

int receive_ymodem_batch(char *directory, char **filenames, int *count, int mode,struct transfer_struct *this_xfer)
{
  int i = 0;
  int abort = 0;
  int total = *count;
  ...
  *count = 0;
  while (!last_file)
  {
    if (i<total)
    {
      if (receive_ymodem_protocol(directory, filenames[i], mode, &last_file,this_xfer))
      {
        abort = 1;
        break;
      }
    ...
    {
      if (receive_ymodem_protocol(directory, temp_fl, mode, &last_file,this_xfer))
      {
        abort = 1;
        break;
      }
...
int send_files(char **filenames, int count, int mode)
{
    ...
    int error;
    char s[50];
    struct transfer_struct this_xfer;

    if (is_console_node(tswitch)) return (0);

    this_xfer.protocol=mode;
    this_xfer.total_blocks=0;
    this_xfer.current_block=0;
    this_xfer.cps=0;
    this_xfer.event=PROTO_NOT_STARTED;
    this_xfer.num_errors=0;
    this_xfer.direction=FROM_CHAT;
    line_status[tswitch].transfer_pid=MY_PID();
    line_status[tswitch].transfer = &this_xfer;
    ...
    if (mode<Y_MODEM_PROTOCOL)
    {
      ...
      error = send_xmodem_protocol(*filenames,length,&this_xfer);
    ...
    {
      error = send_ymodem_batch(filenames,count,mode,&this_xfer);
    }

   line_status[tswitch].transfer = NULL;
   return (error);
}
...
int receive_files(char *directory, char **filenames, int *count, int mode)
{
    ...
    this_xfer.event=PROTO_NOT_STARTED;
    this_xfer.num_errors=0;
    this_xfer.direction=TO_CHAT;
    this_xfer.file_number=0;
    line_status[tswitch].transfer_pid=MY_PID();
    line_status[tswitch].transfer = &this_xfer;
    ...
    if (mode<Y_MODEM_PROTOCOL)
    {
      error = receive_xmodem_protocol(directory,*filenames,crc_mode,mode,&this_xfer);
      *count = error ? 0 : 1;
    }
    else
    {
      error = receive_ymodem_batch(directory,filenames,count,mode,&this_xfer);
    }
