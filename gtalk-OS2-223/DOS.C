...
int getfree(int drive, unsigned long int *freebytes,
                            ...
                            char *pathname, int slashatend, int justadd)
{
  ...
  {
	if (buffer != pathname) pathcat_n(buffer,&pathname,DOS_FILENAME_LEN);
    n = strlen(buffer);
    buffer += n;
  }
  while (*filename)
...
void change_directory(char *data, int id, tfile *files)
{
  ...
  *files->filename = 0;
  ...
  if ((files->filename[1] == ':') && (*files->filename))
  {
    ...
    if (getcurdir(done,files->filename3))
    {
      print_str_cr("Invalid Drive Letter");
      return;
    }
  ...
  done = findfirst(files->filename3,&ourblock,FA_DIREC);
  if (!done)
  {
...
void list_files(char *data, int id, tfile *files)
{
  ...
  *files->filename = 0;
  ...
  done = findfirst(files->new_path,&ourblock,FA_DIREC | FA_HIDDEN);
  while (!done)
  {
  ...
  sprintf(s,"Total Files %d, Total Bytes %lu (%luMB)",totalfiles,totallength,totallength >> 20);
  print_str_cr(s);
  if (!getfree(letter_to_drive(*files->new_path),&totallength,&totalbytes))
  {
	sprintf(s,"Bytes free %lu (%luMB), Total bytes on disk %lu (%luMB)",
		totallength,totallength >> 20,totalbytes,totalbytes >> 20);
    print_str_cr(s);
  }
  print_cr();
}
...
void erase_files(char *data, int id, tfile *files)
{
  ...
  while (notquit)
  {
    *files->filename = 0;
    ...
    if (!done)
	   done = findfirst(files->new_path,&ourblock,0);
    while ((!done) && (notquit))
    {
