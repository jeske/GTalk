...
unsigned long int asctonum(char *t)
{
...
void receive_ymodem(char *data, int id, tfile *files)
{
   ...
   *files->filename = 0;
   ...
   if (*files->filename)
   {
     compose_pathname(files->new_path,files->filename,0);
     print_str_cr(files->new_path);
     recv_files(files->new_path,filepointers,&num_names,3);
   } else
   {
     print_str_cr(files->path);
     recv_files(files->path,filepointers,&num_names,3);
   }
}
