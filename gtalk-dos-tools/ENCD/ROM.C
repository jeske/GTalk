...
void main(void)
{
  ...
  else
  {
    ...
    if (version_no & 0x80)
      {printf("                                   *** CONSOLE VERSION ***\n");
       real_version_no=(version_no & 0x7F);
      }
    else
     { printf("                                   *** SERIAL PORT VERSION ***\n");
       real_version_no=(version_no);
     }

    printf("Version No: %u  (%u)\n",real_version_no,version_no);
  }
