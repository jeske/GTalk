...
void make_default_classes(char *filename)
{
    ...
    if (sizeof(struct class_data_record) < sizeof(struct class_data))
     {
	printf("Class Data struct too large for class_data_record block\n");
	exit(1);
     }
   printf("Class record size: %d(%d used)\n", sizeof(struct class_data_record),
