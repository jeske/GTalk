...
#define GETOPTARGS "n:p:" /* Allowable command line arguments */

/* Define a buffer to hold longjmp state info */
jmp_buf jmp_signal_handler;
...
void main(int argc, char **argv)
{
  ...
  struct modem_dev_st *modems = modemcfg;
	...
	fprintf (stdout, "\nGtalk Server v%s Revision %s.\nInitializing...\n\n",
	         GTALK_VERSION, GTALK_REVISION);

	while ((cargs = getopt(argc, argv, GETOPTARGS)) != EOF)
		{
		switch (cargs)
			{
			case 'n':
				params.nodaemon = TRUE;
				params.console  = TRUE;
				fprintf (stdout, "Setting no daemon flag\n", port_str);
				break;
			case 'p':
				port_str = strdup(optarg);
				fprintf (stdout, "Setting port to [%s]\n", port_str);
				break;
			default:
				fprintf (stderr, "Bad args: [%s] [%c]\n", argv[0], cargs);
				break;
			}
		}
	...
	if (port_str == (char *)NULL)
		{
		fprintf (stderr, "Warning: No port specified\n");
		port_str = strdup ("4000");
		fprintf (stderr, "Defaulting to port [%s]\n", port_str);
		}
	else
		{
		fprintf (stderr, "Server: Using port [%s]\n", port_str);
		}

	gtalk_accept_port = (unsigned)atoi (port_str);
	fprintf (stderr, "Parent PID is [%d]\n", getpid());

	if (params.nodaemon)
		{
		fprintf (stderr, "Server: Not daemonizing via command line switch\n");
		}
	else
		{
		fprintf (stderr, "Server: daemonizing...\n");
		if (become_daemon() < 0)
			{
			log_error ("Couldn't daemonize");
			fprintf (stderr, "Fatal: Couldn't daemonize\nExiting...\n");
			exit (1);
			}
		else
			{
			fprintf (stderr, "Server: Successful daemonization\n");
			}
		}

	i_jump_signal_handler = setjmp (jmp_signal_handler);
	if (i_jump_signal_handler != 0)
		{
		fprintf (stderr, "setjmp returned [%d]\n", i_jump_signal_handler);
		fprintf (stderr, "Server: Shutting down\n");
		fprintf (stderr, " - Closing main port [%d]: ", gtalk_accept_port);
		if (close (accept_fd) == 0)
			fprintf (stderr, "Success\n");
		else
			fprintf (stderr, "FAILED\n");

		fprintf (stderr, " - Closing extern port [%d]: ", gtalk_accept_port+1);
		if (close (extrn_accept_fd) == 0)
			fprintf (stderr, "Success\n");
		else
			fprintf (stderr, "FAILED\n");
  ...
  init_state_list(default_parent_state_list, &parent_state_list);
	...
	if (params.console)
		fprintf (stderr, "Initializing channels: ");
	...
	if (params.console)
		fprintf (stderr, "Creating schedule list: ");
  ...
  while (modems->dev_name)
   {
	...
	if ((accept_fd = bind_server_to(gtalk_accept_port)) < 0)
		...
		fprintf (stderr,
		         "Could not bind telnet to port [%d]\n",gtalk_accept_port);
		exit(1);
		}
	else
		{
		fprintf (stderr, "Binded telnet to port [%d]\n", gtalk_accept_port);
		}

	if ((extrn_accept_fd = bind_server_to(gtalk_accept_port+1)) < 0)
		{
		log_error("Could not bind external to port [%d]",gtalk_accept_port+1);
		fprintf (stderr,
		         "Could not bind externel to port [%d]\n",gtalk_accept_port+1);
		exit(1);
		}
	else
		{
		fprintf (stderr, "Binded to external port [%d]\n", gtalk_accept_port+1);
		}
