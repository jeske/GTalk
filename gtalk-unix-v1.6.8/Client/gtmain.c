...
int pass_prompt(void)
{
  ...
  while ((num_tries_left--) && (!valid_login))
    {
      fflush(stdout);
      printf("\r\n\n\007Press <RETURN> for Guest access.\r\n");
      printf("\r\nUser ID: ");
	...
	{ 
	  if (setup_guest_login(mynode))
	    ...
	    ansi_state = ansi_on(1);
 	    printf_ansi("|*r1\r\nEnter Handle: ");
            get_input_cntrl(mynode->userdata.user_info.handle,GUEST_HANDLE_LEN,
...
void save_online_user(void)
{
 ...
 switch(mynode->sigusr1_action)
   ...
   case SIG1_ACTION_TIMEOUT:
     ...
     mynode->userdata.user_info.stats.time_total += 
             (time(NULL)-mynode->userdata.online_info.login_time);
     mynode->userdata.user_info.stats.time_day +=
             (time(NULL)-mynode->userdata.online_info.login_time);
     mynode->userdata.user_info.stats.time_month +=
             (time(NULL)-mynode->userdata.online_info.login_time);
     break;
   ...
   case SIG1_ACTION_KILL:
     log_event("log/signout.log","|*f1|*h1Kill|*r1   | %s:%s|*r1",
	       s,mynode->userdata.user_info.handle);
     mynode->userdata.user_info.killedstats.kills_total++;
     mynode->userdata.user_info.stats.time_total += 
             (time(NULL)-mynode->userdata.online_info.login_time);
     mynode->userdata.user_info.stats.time_day +=
             (time(NULL)-mynode->userdata.online_info.login_time);
...
void process_command(char *cstring)
{
  ...
  if (*cstring == '/')
	...
	if (testFlag(mynode,"CMD_LURK")) {
	   printf("--> Stealth mode not implemented yet\r\n");
	   stealth_mode = 1;
	}
	*cstring++;
    }
  if ((!(com=find_command(&cstring))) || ((com->flag_name) && (!testFlag(mynode, com->flag_name))))
    {
...
void updatestats(void)
{
  ...
  time_t curtime = time(NULL);  
   ...
   if (!mynode->userdata.user_info.last_call) 
    {  mynode->userdata.user_info.stats.calls_month=0;
       mynode->userdata.user_info.stats.time_month=0;
       mynode->userdata.user_info.stats.calls_day=0;
       mynode->userdata.user_info.stats.time_day=0;
    } 

  temp_time = localtime(&mynode->userdata.user_info.last_call);
  strftime(buf1,6,"%Y",temp_time); /* test for different year (with century) */
  temp_time = localtime(&curtime);
  strftime(buf2,6,"%Y",temp_time);
  ...
  {
    temp_time = localtime(&mynode->userdata.user_info.last_call);
    strftime(buf1,6,"%m",temp_time);  /* test for different month */
    temp_time = localtime(&curtime);
    strftime(buf2,6,"%m",temp_time);
    ...
    {
      temp_time = localtime(&mynode->userdata.user_info.last_call);
      strftime(buf1,6,"%d",temp_time); /* test for different day */
      temp_time = localtime(&curtime);
      strftime(buf2,6,"%d",temp_time);
      if (strcmp(buf1,buf2)) {
        mynode->userdata.user_info.stats.calls_day=0;
        mynode->userdata.user_info.stats.time_day=0;
      }
  ...
  mynode->userdata.user_info.stats.calls_month++;
  mynode->userdata.user_info.stats.calls_day++;
  mynode->userdata.user_info.stats.calls_total++;
...
void main_loop(void)
{
  ...
  if(*mynode->ip) {
   printf_ansi("|*f2|*h1Your IP is: |*r1%s\n\r",mynode->ip);
  }
  updatestats();
  ansi_on(0);
  ml_logout = 0;
	  ...
	  if (FD_ISSET(0, &read_fd))
	    {
	      ...
	      if (*s == '/')
		process_command(s);
	      else if (*s == '~' && testFlag(mynode,"MIL_ACTIONS"))
		{
		  if (*mynode->cur_chan)
		      /* write_to_channel(mynode->cur_chan,"Coming soon to an NG near you: ~ (Action Commands)"); */
		      printf_ansi("--> Not yet implemented. It's coming soon, though... :-)\r\n");
		  else
		      printf_ansi("--> No current channel\r\n");
		}
	      else if (*s == '`' && testFlag(mynode,"CMD_WALL"))
		{
		  cmd_walla_message(NULL, s);
		}
	      else if (*s == '!' && testFlag(mynode,"CMD_LURK"))
		  cmd_lurk_message(NULL, s);
	      else if (*s)
		{
		  if (*mynode->cur_chan)
		    {
...
int check_for_lock(void)
{
     ...
     case TELNET_NODE_TYPE:
        if ((mynode->userdata.online_info.class_info.priority >
           ...
           (c_sys_info->lock_priority_telnet)) {
                printf("\nTelnet Lock Priority [%d]\r\n",
                   c_sys_info->lock_priority_telnet);
                if (mynode->userdata.user_info.enable) {
                   print_file("text/locked-telnet.txt");
                } else {
     ...
     case MODEM_NODE_TYPE:
        if ((mynode->userdata.online_info.class_info.priority >
           ...
           (mynode->userdata.online_info.class_info.priority) &&
           (c_sys_info->lock_priority)) {
                printf("\nModem Lock Priority [%d]\r\n",
                   c_sys_info->lock_priority);
                if (mynode->userdata.user_info.enable) {
                   print_file("text/locked-modem.txt");
                } else {
                   print_file("text/glocked.txt");
                }
        exit(0);
...
void print_login_info(void)
{
  ...
  if (stat(spoolfile,&spool_stat)) {
    ...
    if (spool_stat.st_mtime > spool_stat.st_atime)
      printf_ansi("|*fc*** You have |*f9NEW|*fc Email!|*r1\n");
    else
