 ...
 if ((child_pid = fork())==0)
   {
	...
	{ perror("STDIN");}
     ...
     if (!(options & COM_NOCHROOT)) {
       if (chroot("/home/gtalk/ROOT")) {
	 perror("/home/gtalk/ROOT");
...
int cmd_change_channel(com_struct *com, char *string)
{
  ...
  if (!(*channel))
    {
      printf_ansi("--> No channel to change to\n\r");
      return (-1);
    }
  remove_ansi(channel);
...
int cmd_system_list(com_struct *com, char *string)
{
  ...
  for(node_count=0;node_count<MAX_NODES;node_count++)
	...
	case NODE_ACTIVE:
     ...
     if (c_nodes(node_count)->cur_chan[0])
       printf(" (%s) ", c_nodes(node_count)->cur_chan);
     else 
       printf(" - ");
  ...
  if (c_sys_info->lock_priority) {
    printf_ansi("--> System Locked to Priority [%d]\n",
 ...
 if (get_number(&string,&num))
   {
     ...
     printf("--> System Locked to Priority [%d]\r\n",num);
  ...
  if (!get_number(&string, &num))
    {
      ...
      print_file("/home/gtalk/ROOT/home/gtalk/rotator/index.hdr");
      print_file("/home/gtalk/ROOT/home/gtalk/rotator/index.txt");
      return 0;
    }
  
  ansi_on(1);
  printf_ansi("|*ff--> Message Box [|*r1%03d|*ff]|*r1\n",num);
  sprintf(path,"%s/home/gtalk/rotator/rot%03d","/home/gtalk/ROOT",num);
  print_file_cntrl(path,PFC_ANSI);
...
int cmd_enter_bbs(com_struct *com, char *string)
  ...
  exec_user_program("/home/gtalk/bin/bbs", mynode, COM_NOCHROOT);
}
...
/* sprintf(tmp,"/usr/bin/chfn %s", mynode->userdata.user_info.login); */
   ...
   printf("Enter the user number to change (###): ");
