...
struct ext_video       /* Structure to keep track of special ANSI */
 {             /* codes */
   ...
   char see_color;     /* = see colors or not */
   char ncurses_sc;    /* = show an ncurses screen */
 };

static struct ext_video vids = { 0, 0, 0, 0, 0, 0, 0, 1,0 };


void set_term_ansi(int state)
{
  vids.see_color = state;
}

void set_term_ascii(int state)
{
  vids.see_controls = !state;
}

int get_term_ansi()
{ return (vids.see_color); }

int get_term_ascii()
{ return (!vids.see_controls);}
...
void print_chr_noflush(char temp)
{
  ...
  if (vids.terminal)    /* If the ANSI codes are turned on */
	...
	{
	case 0: 
	  if (temp == START_ANSI_CHAR) /* If its the start char */
	    {
	...
	case 2: 
	  if (temp > 'Z') temp -= 32;  /* Make command uppercase */
	  if ((temp<'A') || (temp>'Z')) 
	    vids.stage = 0;
	  else {
	    if (vids.see_color)
	      vids.stage = temp;	/* and next stage should be */
	    else
	      vids.stage = 3;           /* just ignore the color code */
	  }
	 
	  return;         /* the actual letter */
	case 3: /* color is off so just ignore the code when it
                   comes through */
	  vids.stage = 0;
	  return;
	case 7: 
	  vids.curchr = chex_digit(temp);
	  vids.stage = 8;
	  return;
	case 8: vids.stage = 0;
	  if (vids.curchr == -1)
	...
	case 'B': 
	  if ((temp>='0') && (temp<='7'))	/* if its background */
	  {
...
int wait_for_return(void)
{
  int count;
  int abort=0;
  static char paging_text[] = "[ Press Return ]";

    print_string(paging_text);
    do
     {
       count = wait_ch();
       if (((count == 27) || (count == 3)))
  ...
  repeat_chr(8,strlen(paging_text),0);
  return abort;
}
  ...
  while (*string)
	...
	if (*ansi_strp=='|') {
	  if ( ((*(ansi_strp+1)=='*') || (*(ansi_strp+1)=='+')) 
	    ...
	    if (*(ansi_strp+1)=='+') {     
	                       /* if it's High ASCII count it as 1*/
	      if (dif>3)
		dif-=3;
	      else dif=1;
	    } else {
	                       /* if it's ANSI count it as 0 */
	      if (dif>4)
		dif-=4;
	      else dif=1;
	    }
	    ansi_strp+=4;      /* increment the walker */
	  } else {
	    ansi_strp++;
	  }
	} else {
