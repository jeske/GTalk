...
int load_base(struct mail_pass_data *data, int create, int mode)
{
  ...
  {
   default:
   case R_BASE  : mode_index = 1;
                  break;
   case RW_BASE : mode_index = 0;
                  break;
  }
  ...
  if (!(data->fp=g_fopen(filename,modes[mode_index],"LOADBASE")))
  {
    if (!create)
       {if (flag) unlock_dos();
        return (0);
...
{
  if (data->open_mode==RW_BASE) flush_base(data);
  g_fclose(data->fp);
  data->is_open=0;
}
...
{                 /* Current user #=user_lines[tswitch].number */
  ...
  data->mail = user_no;
  return (load_base(data,create_mail));
}
...
{                 /* Current user #=user_lines[tswitch].number */
  ...
  data->mail = -1;
  return (load_base(data,create_bbs));
}

int read_bbs_base(int bbs_no, struct mail_pass_data *data, int create_bbs)
{  int ret_val= open_bbs_base(bbs_no,data,create_bbs);
  g_fclose(data->fp);
  data->is_open=0;
  return ret_val;
...
{  int ret_val= open_mail_base(user_no,data,create_mail);
  g_fclose(data->fp);
  data->is_open=0;
  return ret_val;
}
