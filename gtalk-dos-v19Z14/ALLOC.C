...
#define DAVE_FUDGE_FACTOR 32768l  // leave this much for DOS
  ...
  while (current < mem_handles)
  {
    if (cur_entry->empty)
    {
      if ((cur_entry->paragraphs>=size))
      {
        ...
        {
          if (use_ems == EMS_MEMORY) flag = 0;
            else flag = ((!cur_entry->in_ems) && (!cur_block_in_ems) && is_smallest);
        }
        if (flag)
        {
         ...
         int for_task, int use_ems, int owner)
{
    ...
    memory_pointer = allocate_some_memory(memory,description,owner,use_ems,for_task);
#ifdef DEBUG
    sprintf(s,"Mallocing memory pointer %p",memory_pointer);
#endif
    if (tasking)
...
int checkblacklist(char *filename)
{
