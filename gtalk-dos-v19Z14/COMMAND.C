...
struct command_list far commands[NUMCOMMANDS] = {
                                       ...
                                       {"B",message_bot,HANDLE_PRV,1,0,"",0,0},
                                       {"BBS",bbs_system,BBS_PRV,1,'B',"BBS",0,0},
                                       {"BOT",print_bot_list,HANDLE_PRV,1,0,"",0,0},
                                       ...
                                       {"COLOR",set_reset_color,HANDLE_PRV,1,0,"",0,1},
                                       ...
                                       {"DS",double_space_toggle,HANDLE_PRV,1,0,"",1,1},
                                       ...
                                       {"RE",set_return_echo,HANDLE_PRV,1,0,"",0,1},
                                       ...
                                       {"SLOW",slowdown_text,HANDLE_PRV,1,0,"",1,1},
                                       ...
                                       {"SYSHELP",file_section,S_PRV,1,'H',"Help Section",0,0},
                                       ...
                                       {"V",validate_user,VALIDATE_PRV,1,0,"",1,1},
                                       {"W",set_width,HANDLE_PRV,1,0,"",1,1},
                                       ...
                                       {"X",squelch_node,PRIVATE_PRV,1,0,"",1,1}
                                    };
...
void show_task_info(char *str,char *name,int portnum)
{
  ...
  unlock_dos();
  repeat_chr('-',68,1);
  print_str_cr("PID    Num    STKOFF   STKSEG   EMS   EMSH   PAUSED    TIME     NAME");
  repeat_chr('-',68,1);
  for (loop=0;loop<MAX_THREADS;loop++)
   if (tasks[loop].status)
    {
      sprintf(s,"%06u [%02d]    %04X     %04X     %u    %04u     %u  ",tasks[loop].pid,
...
void system_information(char *str,char *name,int portnum)
{
    ...
    print_string("|*h1Software Version: |*r1");
    print_str_cr(version_title);
    print_string("|*h1    Compile Time: |*r1");
    print_string(compile_time);
    repeat_chr(' ',5,0);
    print_str_cr(compile_date);
    print_string(  "|*h1Operating System: |*r1");
    print_str_cr(os_name);
    print_string("|*h1     System Load: |*r1");
    ...
    print_str_cr(s);
    sprintf(s,"     |*h1Calls Today:|*r1 %04lu                   |*h1Calls Total:|*r1 %06lu",sys_info.day_calls.total,sys_info.calls.total);
    print_str_cr(s);
    sprintf(s,"|*h1 Yesterday Calls:|*r1 %04lu                   |*h1Call Record:|*r1 %04lu",sys_info.yesterday_calls.total,sys_info.record_calls.total);
    print_str_cr(s);
    sprintf(s,"|*h1Calls This Month:|*r1 %04lu              |*h1Calls Last Month:|*r1 %04lu",sys_info.month_calls.total,sys_info.last_month_calls.total);
    sprint_time(t1,&sys_info.last_uptime);
    print_str_cr(s);
    sprintf(s,"     |*h1Last Uptime:|*r1 %s",t1);
    print_str_cr(s);

    sprint_time(t1,&sys_info.down_time);
    sprintf(s,"        |*h1Downtime:|*r1 %s",t1);
    print_str_cr(s);
    print_string("|*h1Last Running For: |*r1");
    print_expanded_time_cr(sys_info.down_time-sys_info.last_uptime);
    print_string("        |*h1Down For: |*r1");
    print_expanded_time_cr(sys_info.uptime-sys_info.down_time);

    sprint_time(t1,&sys_info.uptime);
    sprintf(s,   "|*h1          Uptime:|*r1 %s",t1);
    print_str_cr(s);
    ...
    print_string("|*h1     Running For: |*r1");
...
void show_members(char *str, char *name, int portnum)
{
 ...
 if (*str)
  ...
  if (plusflag && test_bit(user_lines[tswitch].privs,VIEW_USER_DATA_PRV))
   {
...
void channel_list(char *str,char *name,int portnum)
{
   ...
   print_str_cr(s);
   print_str_cr("-------------------------------------------------------------------");
   print_str_cr("Num  Name              Priority | Public | /C Messages | Moderation");
   print_str_cr("-------------------------------------------------------------------");
   print_string("[01]:");
   ...
   if (user>sys_info.max_nodes)
     {
        ...
        if (((bot_num = find_bot_pid(pid_of(user))) == -1) || !abuf_status[user].active)
         {
            print_sys_mesg(NotOnLine);
            return;
         }
