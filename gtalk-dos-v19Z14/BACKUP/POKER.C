  ...
  return;
    ...
    if (players[player_no].money<(amount - players[player_no].current_bet))
     ...
     if (!current_bet)
       ...
       sprintf(s,"%s|*r1 raises %lu to make the bet %lu.",(char far *)players[player_no].handle,(unsigned long int)(amount-current_bet),(unsigned long int)amount);
  ...
  for (count=0;count<num_players;count++)
    {
        ...
        if (players[count].credit)
           sprintf(s,"#%02d:%s|*r1 with -% 9lu money  at run_level:%s",players[count].portnum,players[count].handle,players[count].credit - players[count].money,
                  run_levels[players[count].run_level]);
        else
           sprintf(s,"#%02d:%s|*r1 with % 9lu money  at run_level:%s",players[count].portnum,players[count].handle,players[count].money,
                  run_levels[players[count].run_level]);
